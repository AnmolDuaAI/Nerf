...
from Nerfs.embedder import get_embedder
from Nerfs.NerfModel import NeRF
import os
...
def create_nerf():
    ...
    if len(ckpts) > 0 and not config.no_reload:
        ...
        ckpt = torch.load(ckpt_path) # Loading data from checkpoint

        start = ckpt['global_step'] # Global step
        optimizer.load_state_dict(ckpt['optimizer_state_dict']) # Loading Optimizer state dict

        # Load model
        model.load_state_dict(ckpt['network_fn_state_dict']) # Loading Model Dict
        if model_fine is not None:
            model_fine.load_state_dict(ckpt['network_fine_state_dict']) # Loading Model Fine Dict
